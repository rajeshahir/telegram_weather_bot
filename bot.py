...
def build_forecast(lat, lon, tz, date_str, start, end, models):
    ...
    for m in models:
        ...
        df.columns = [f"{c}_{m}" for c in df.columns]
        dfs.append(df)
...
def plot_forecast(forecast, models, out_path="forecast.png"):
    ...
    plt.subplot(3,1,1)
    for m in models:
        plt.plot(forecast["time"], forecast[f"temp_{m}"], marker="o", linestyle="-", label=m)
    ...
    plt.subplot(3,1,2)
    for m in models:
        plt.plot(forecast["time"], forecast[f"precip_{m}"], marker="s", linestyle="--", label=m)
    ...
    plt.subplot(3,1,3)
    for m in models:
        plt.plot(forecast["time"], forecast[f"wind_{m}"], marker="^", linestyle="-.", label=m)
    ...
    try:
        if len(context.args) < 7:
            await update.message.reply_text(
                "Usage: /forecast <lat> <lon> <timezone> <YYYY-MM-DD> <start_hr> <end_hr> <models>"
            )
            return
        ...
        if not models:
            await update.message.reply_text("No valid models. Use /models")
            return

        fc = build_forecast(lat, lon, tz, date_str, start, end, models)
        txt = fc.to_string(index=False)

        if len(txt) > 3800:  # Telegram message limit
            ...
            await update.message.reply_text(f"<pre>{snippet}</pre>", parse_mode="HTML")
        else:
            await update.message.reply_text(f"<pre>{txt}</pre>", parse_mode="HTML")

        png = "forecast.png"
        plot_forecast(fc, models, png)
